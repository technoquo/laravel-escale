    ...
    <main>
        <div class="container py-5">
            <div class="text-center mb-5">
                <h2 class="fw-bold">Convention</h2>
            </div>

            @forelse ($conventions as $convention)
                <div class="mb-5">
                    <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
                        <h4 class="fw-semibold mb-0">{{ $convention->title }}</h4>
                        @if ($convention->pdf)
                            <a href="{{ $convention->pdf }}" target="_blank" class="btn btn-outline-primary btn-sm">
                                <i class="fa fa-file-pdf me-1"></i> Télécharger PDF
                            </a>
                        @endif
                    </div>

                    @foreach ($convention->items as $item)
